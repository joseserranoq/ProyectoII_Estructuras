...
void leer()
{
	...
	while (!archivo.eof()) {//end of file
		cout << "\nNombre: " << p.nombre << " Edad: " << p.edad << " Genero: " << p.genero << " Estado Civil: " << p.estadoCivil
			<< " Oficio: " << p.oficio << " Sueldo: " << p.sueldo << " Aï¿½os de trabajo: " << p.anosTrabajo << " Cantidad de hijos: " << p.cantHijos
...
int buscaPersona(string nombre)
{
	...
	cout << "No se encontrï¿½ el nombre" << endl;
	archivo.close();
}
...
void modificarPersona(Persona modificar, int posicion){
	...
	cout << "No se encontro la persona";     //SE IMPRIME AUNQUE LA PERSONA YA ESTE AGREGADA ï¿½DEJAR MSJ O NO?
	archivo.close();
...
void datosQuemados() {
	...
	//in order
	//nombre, edad, genero, estadocivil, oficio, sueldo, aï¿½os de trabajo, cant hijos, hobby, alimentaciï¿½n, comida, musica, provincia, canton , distrito

	Persona p1 = { "Ana Alves", 20, "Mujer", "Soltera", "Miscelanea", 500000, 3, 1, "Futbol", "Pollotariano", "Salado", "Hardcore zumba","Alajuela","San Ramï¿½n","Palmares", 1 };
	escribir(p1);
	...
	escribir(p3);
	Persona p4 = {"Alina Acuï¿½a", 25, "mujer", "Soltera", "Enfermera", 700000, 5, 2, "Tenis", "Lactovegetariano", "Agridulce", "Merengue","Alajuela","Grecia","Bolivar", 1};
	escribir(p4);
	...
	escribir(p17);
	Persona p18 = { "Sofia Salas", 19, "Mujer", "Soltera", "Recepcionista", 500000, 1, 0, "Ver series", "Omnivoro", "Picante", "Caribeï¿½o","Alajuela","San Carlos","Agua Azul", 1 };
	escribir(p18);
	...
	escribir(p34);
	Persona p35 = { "Juana La Cubana", 50, "Mujer", "Soltera", "Escritora", 600000, 25, 0, "Ver novelas", "Pollotariano", "Pollo", "Reggae","Alajuela","San Ramï¿½n","Bajo Rodriguez", 3 };
	escribir(p35);
	...
	escribir(p39);
	Persona p40 = { "Gledys Huertas", 55, "Mujer", "Viuda", "Diseï¿½adora", 500000, 10, 0, "Ver revistas de moda", "Omnivoro", "Dulce", "Jazz","San Jose","Desamparados","San Miguel", 1 };
	escribir(p40);
	...
	escribir(p45);
	Persona p46 = { "Pepe Jose", 20, "Hombre", "Soltero", "Mesero", 600000, 1, 0, "Ver novelas", "Vegetariano", "Pescado", "Reggae","Cartago","San Ramï¿½n","Bajo Rodriguez", 3 };
	escribir(p46);
...
int main()
{
  ...
  //El metodo leerPersona, imprime toda la imformacion de la persona.
	...
	//cout << "El tamaï¿½o de la estructura:  " << sizeof(Persona);
	return 0;
}
